...
local default_cfg = {
    toggles = { balloon = true, bodycam = true, unfreeze = true, fisheye = true, hand = true, limit = true, anim68 = true, autosport = false, autodrift = false, antibpwb = false, oldesc = true, dlgstyle_enabled = true, autotax = false, femboy = false },
...
local femboy_dict = {
    ["привет"]="приветик", ["Привет"]="Приветик", ["пока"]="поки-чмоки", ["Пока"]="Поки-чмоки",
    ["да"]="агась", ["Да"]="Агась", ["спасибо"]="сябочки", ["Спасибо"]="Сябочки",
...
local femboy_actions = {
    "/me мило улыбнулся", "/me поправил волосы", "/me смущенно отвел взгляд",
    "/me сделал жест пальцами в виде сердечка", "/me нежно прижался ближе к человеку напротив",
    "/me робко взял за руку", "/me густо покраснел и опустил глазки",
    "/me ласково потерся щечкой о плечо", "/me тихо хихикнул, прикрыв ротик ладошкой"
}
...
local FIX_JS = [[
try {
  if (window && window.cef && typeof window.cef.HandleGameMenu === 'function') { window.cef.HandleGameMenu(false); }
  if (typeof window.executeEvent === 'function') { window.executeEvent('event.mainMenu.setMainMenuDisabled', `[true]`); }
} catch (e) {}
]]
...
function doPayTaxes()
    ...
    if not waitForDialog(6565, 5000) then return sampAddChatMessage("{FF6060}[MultiTool] Ошибка: телефон не открылся", -1) end
    wait(300)
    sampSendDialogResponse(6565, 1, 4, "")
    
    local dId = waitForAnyDialog(5000)
    if not dId then return sampAddChatMessage("{FF6060}[MultiTool] Ошибка: диалог налогов не появился", -1) end
    ...
    if not waitForDialog(6565, 5000) then
        wait(500) cefSend("launchedApp|24")
        if not waitForDialog(6565, 5000) then return sampAddChatMessage("{FF6060}[MultiTool] Ошибка: меню телефона не открылось", -1) end
    end

    wait(300)
    sampSendDialogResponse(6565, 1, 12, "")
    if not waitForDialog(26155, 5000) then return sampAddChatMessage("{FF6060}[MultiTool] Ошибка: диалог отеля не появился", -1) end

    wait(300)
    local h_time = sampGetDialogText():match("(%d+)%s")
    ...
    if not waitForDialog(27806, 5000) then return sampAddChatMessage("{FF6060}[MultiTool] Ошибка: диалог квартиры", -1) end
    wait(300)
    local f_tax = sampGetDialogText():match("%$([%d,%.]+)")
    f_tax = f_tax and f_tax:gsub("[,%.%s]", "") or nil

    if f_tax and tonumber(f_tax) > 0 then
        sampSendDialogResponse(27806, 1, 0, f_tax)
        sampAddChatMessage("{90EE90}[MultiTool] Налог на квартиру оплачен: $" .. f_tax, -1)
    else
        ...
        if eventName == "event.radialMenu.items" then
            ...
            for _, item in ipairs(items) do
                ...
                if isAutomatingSport then
                    if title:find("sport") or title:find(u8:encode("спорт")) then targetId = item.id or item.uid
                ...
                elseif isAutomatingDrift then
                    if title:find("drift") or title:find(u8:encode("дрифт")) then targetId = item.id or item.uid end
                end
...
local function GenerateTextDraw(id, PosX, PosY)
    if id == 0 then
        t = t + 1; sampTextdrawCreate(t, "LD_SPAC:white", PosX - 5, PosY + 7); sampTextdrawSetLetterSizeAndColor(t, 0, 3, 0x80808080); sampTextdrawSetBoxColorAndSize(t, 1, 0xFF759DA3, PosX+5, 0.0)
    elseif id == 1 then
        for i = 0, 1 do t = t + 1; sampTextdrawCreate(t, "LD_SPAC:white", PosX - ((i == 0) and 3 or -3), PosY); sampTextdrawSetLetterSizeAndColor(t, 0, 4.5, 0x80808080); sampTextdrawSetBoxColorAndSize(t, 1, 0xFF759DA3, PosX-((i == 0) and 15 or -15), 0.0) end
    elseif id == 2 then
        for i = 0, 1 do t = t + 1; sampTextdrawCreate(t, "LD_SPAC:white", PosX - ((i == 0) and -8 or 6), PosY + ((i == 0) and 7 or 25)); sampTextdrawSetLetterSizeAndColor(t, 0, 0.8, 0x80808080); sampTextdrawSetBoxColorAndSize(t, 1, 0xFF759DA3, PosX-((i == 0) and 15 or -15), 0.0) end
    elseif id == 3 then
        for i = 0, 1 do t = t + 1; sampTextdrawCreate(t, "LD_SPAC:white", PosX+10, PosY+((i == 0) and 7 or 25)); sampTextdrawSetLetterSizeAndColor(t, 0, 1, 0x80808080); sampTextdrawSetBoxColorAndSize(t, 1, 0xFF759DA3, PosX-15, 0.0) end
    elseif id == 4 then
        for i = 0, 1 do t = t + 1; sampTextdrawCreate(t, "LD_SPAC:white", PosX - ((i == 0) and -10 or -10), PosY + ((i == 0) and 0 or 25)); sampTextdrawSetLetterSizeAndColor(t, 0, (i == 0) and 1.8 or 2, 0x80808080); sampTextdrawSetBoxColorAndSize(t, 1, 0xFF759DA3, PosX-((i == 0) and 10 or 15), 0.0) end
    elseif id == 5 then
        for i = 0, 1 do t = t + 1; sampTextdrawCreate(t, "LD_SPAC:white", PosX - ((i == 0) and 8 or -10), PosY + ((i == 0) and 7 or 25)); sampTextdrawSetLetterSizeAndColor(t, 0, (i == 0) and 0.8 or 1, 0x80808080); sampTextdrawSetBoxColorAndSize(t, 1, 0xFF759DA3, PosX-((i == 0) and -15 or 15), 0.0) end
    elseif id == 6 then
        for i = 0, 1 do t = t + 1; sampTextdrawCreate(t, "LD_SPAC:white", PosX - ((i == 0) and 7.5 or -10), PosY + ((i == 0) and 7 or 25)); sampTextdrawSetLetterSizeAndColor(t, 0, (i == 0) and 0.8 or 1, 0x80808080); sampTextdrawSetBoxColorAndSize(t, 1, 0xFF759DA3, PosX-((i == 0) and -15 or 10), 0.0) end
    elseif id == 7 then
        t = t + 1; sampTextdrawCreate(t, "LD_SPAC:white", PosX - 13, PosY + 7); sampTextdrawSetLetterSizeAndColor(t, 0, 3.75, 0x80808080); sampTextdrawSetBoxColorAndSize(t, 1, 0xFF759DA3, PosX+5, 0.0)
    elseif id == 8 then
        for i = 0, 1 do t = t + 1; sampTextdrawCreate(t, "LD_SPAC:white", PosX+10, PosY+((i == 0) and 7 or 25)); sampTextdrawSetLetterSizeAndColor(t, 0, 1, 0x80808080); sampTextdrawSetBoxColorAndSize(t, 1, 0xFF759DA3, PosX-10, 0.0) end
    elseif id == 9 then
        for i = 0, 1 do t = t + 1; sampTextdrawCreate(t, "LD_SPAC:white", PosX+10, PosY+((i == 0) and 6 or 25)); sampTextdrawSetLetterSizeAndColor(t, 0, 1, 0x80808080); sampTextdrawSetBoxColorAndSize(t, 1, 0xFF759DA3, PosX-((i == 0) and 10 or 15), 0.0) end
    end
end
        ...
        if imgui.BeginChild("MainContentArea", imgui.ImVec2(0, -28), false) then
            ...
            if imgui.BeginTabBar("MultiMenuTabs", imgui.TabBarFlags_FittingPolicyResizeDown) then
                ...
                if imgui.BeginTabItem(u8"Функции", nil, 0) then
                    ...
                    drawCb("Global FOV", "fisheye") drawCb("Hand-Run", "hand") drawCb("Limit", "limit") drawCb("Фембой режим (ня~)", "femboy")
                    imgui.NextColumn()
                ...
                if imgui.BeginTabItem(u8"Биндер", nil, 0) then
                    ...
                    drawBRow("Рука (Hand-Run)", "hand") drawBRow("Лимит скорости", "limit") drawBRow("Sbiv piss", "anim68")
                    ...
                    imgui.Spacing() imgui.Separator() imgui.Spacing()
                    if imgui.Button(u8"Оплатить налоги сейчас", imgui.ImVec2(-1, 24)) then lua_thread.create(doPayTaxes) end
    ...
    while true do
        ...
        if mainIni.toggles.autotax then
            ...
            if current_time - last_autotax_time >= (mainIni.settings.autotax_interval * 60) then
                ...
                if not (sampIsChatInputActive() or sampIsDialogActive()) then lua_thread.create(doPayTaxes) end
            end
...
function sampev.onSendDialogResponse(id, but, lis, input)
    if captcha_dialog_id and id == captcha_dialog_id then
        sampAddChatMessage(string.format("{FFFFFF}Введенная капча: {22A872}[%s]{FFFFFF}, время ввода: {22A872}[%.3f сек]{FFFFFF}", input, os.clock() - real_captcha_start), -1)
